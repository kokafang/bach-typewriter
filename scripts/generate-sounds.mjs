...
function wavFor(freq) {
  ...
  for (let i = 0; i < samples; i += 1) {
    ...
    const release = Math.max(0, 1 - Math.max(0, t - 0.19) / 0.05);
    const body =
    ...
    const click = Math.sin(2 * Math.PI * 2200 * t) * Math.exp(-t * 90) * 0.08;
    const sample = Math.max(-1, Math.min(1, (body * attack * decay * release + click) * 0.58));
